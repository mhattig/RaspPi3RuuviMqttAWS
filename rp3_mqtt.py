'''
 ...
 publishes are performed. One pub message contains the data from a 
 single Ruuvi tag advertisment.
...
class Rp3MQTTClient(mqtt.Client):
    """ 
    ...
    connection from a client id at a time.
    ...
    def __init__(self, client_id=client_id, sub_topics=None, logging=logging):
        ...
        try :
            ...
            rc = self.mqttc.tls_set(certfile=cert_filename, keyfile=key_filename, ca_certs=cafile_filename, cert_reqs=ssl.CERT_REQUIRED, tls_version=ssl.PROTOCOL_TLSv1_2, ciphers=None)
    ...
    def on_message(self, mqttc, obj, msg):
        ...
        if self.logging:
            print('on_message: topic = {}, qos = {}, msg = {}'.
    ...
    def on_subscribe(self, mqttc, obj, mid, granted_qos):
        self.sub_flag=True
        if self.logging:
            print('on_subscribe: mid = {}, granted qos = {}'.
                format(mid, granted_qos))

    def on_unsubscribe(self, mqttc, userdata, mid):
        self.sub_flag=False
        if self.logging:
            print('on_subscribe: userdata = {}, mid = {}'.
                format(userdata ,mid))
    ...
    def on_log(self, mqttc, obj, level, string):
        ...
        if self.logging:
            ...
            print('log flags: connect {}, disconnect {}, pub = {}, sub = {}, rcv cnt = {}, pub cnt = {}, '.format(self.connect_flag, self.disconnect_flag, self.pub_flag, self.sub_flag, self.rcv_msg_count, self.pub_msg_count))
    ...
    def ruuvi_pub(self, value):
        mac_addr = value.get('mac', None)
        UTC = value.get('UTC', None)
        if mac_addr and UTC:
            topic = 'ruuvi_tag/{}'.format(mac_addr)
            self.mqtt_pub(topic, value)
        elif self.logging:
            print('Ruuvi tag data missing mac addr or UTC, don\'t pub')

def main():
    """
    Test Raspberry Pi 3 MQTT client that sends Ruuvi Tag data.
    """
    ...
    value1 = {
        ...
        'UTC': '2019-10-29 03:30:30.303030'
    } 
    ...
    value2 = {
        ...
        'measurement_sequence_number': 704, 
        'mac': 'F7:E3:69:3A:DC:5B',
        'UTC': '2019-10-29 03:31:31.313131'
    } 
    value2['UTC'] = str(datetime.datetime.utcnow())
    time.sleep(1)
    value3 = {
        'data_format': 5, 
        'humidity': 32.00, 
        ...
        'measurement_sequence_number': 704, 
        'mac': 'F7:E3:69:3A:DC:5B',
        'UTC': '2019-10-29 03:32:32.323232'
    } 
    value3['UTC'] = str(datetime.datetime.utcnow())
    time.sleep(1)

    # Three values from the same Ruuvi Tag (i.e. mac address)
